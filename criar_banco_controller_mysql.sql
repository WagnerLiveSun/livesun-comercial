-- =====================================================================
-- LiveSun Controller - Script completo de criação de banco (MySQL 8+)
-- =====================================================================
-- Objetivo:
-- 1) Criar database/schema do projeto Controller
-- 2) Criar owner do schema (usuário MySQL dono lógico do banco)
...
-- =========================
-- 0) PARAMETRIZAÇÃO MANUAL
-- =========================
-- Substitua os valores abaixo:
--   controller_db_owner          -> usuário dono do schema
--   Troque a senha forte
...
CREATE USER IF NOT EXISTS 'controller_db_owner'@'%' IDENTIFIED BY 'Troque_Essa_Senha_Forte_2026!';

GRANT ALL PRIVILEGES ON controller.* TO 'controller_db_owner'@'%';
FLUSH PRIVILEGES;
...
CREATE TABLE IF NOT EXISTS fluxo_contas_modelo (
    ...
    CONSTRAINT fk_fluxo_contas_empresa
      FOREIGN KEY (empresa_id) REFERENCES empresas(id),
...
CREATE TABLE IF NOT EXISTS contas_banco (
    ...
    CONSTRAINT fk_contas_banco_empresa
      FOREIGN KEY (empresa_id) REFERENCES empresas(id),
    CONSTRAINT fk_contas_banco_fluxo_conta
      FOREIGN KEY (fluxo_conta_id) REFERENCES fluxo_contas_modelo(id),
...
CREATE TABLE IF NOT EXISTS lancamentos (
    ...
    CONSTRAINT fk_lancamentos_empresa
      FOREIGN KEY (empresa_id) REFERENCES empresas(id),
    CONSTRAINT fk_lancamentos_entidade
      FOREIGN KEY (entidade_id) REFERENCES entidades(id),
    CONSTRAINT fk_lancamentos_fluxo_conta
      FOREIGN KEY (fluxo_conta_id) REFERENCES fluxo_contas_modelo(id),
    CONSTRAINT fk_lancamentos_conta_banco
...
CREATE TABLE IF NOT EXISTS fluxo_caixa_realizado (
    ...
    CONSTRAINT fk_fluxo_realizado_empresa
      FOREIGN KEY (empresa_id) REFERENCES empresas(id),
    CONSTRAINT fk_fluxo_realizado_fluxo_conta
      FOREIGN KEY (fluxo_conta_id) REFERENCES fluxo_contas_modelo(id),
    CONSTRAINT fk_fluxo_realizado_conta_banco
      FOREIGN KEY (conta_banco_id) REFERENCES contas_banco(id),
...
CREATE TABLE IF NOT EXISTS fluxo_caixa_previsto (
    ...
    CONSTRAINT fk_fluxo_previsto_empresa
      FOREIGN KEY (empresa_id) REFERENCES empresas(id),
    CONSTRAINT fk_fluxo_previsto_fluxo_conta
      FOREIGN KEY (fluxo_conta_id) REFERENCES fluxo_contas_modelo(id),
    CONSTRAINT fk_fluxo_previsto_conta_banco
      FOREIGN KEY (conta_banco_id) REFERENCES contas_banco(id),
...
-- =======================================
-- 3) CONCILIAÇÃO BANCÁRIA (NOVO PROJETO)
-- =======================================
CREATE TABLE IF NOT EXISTS conciliacao_bancaria (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'aberta', -- aberta, fechada, revisao
    observacoes TEXT NULL,
...
CREATE TABLE IF NOT EXISTS conciliacao_item (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'pendente', -- pendente, conciliado, divergente
...
-- ====================================================
-- 4) CONTROLE DE ACESSO (RBAC COMPLEMENTAR E AUDITORIA)
-- ====================================================
...
-- =================================
-- 5) DADOS BASE DE CONTROLE DE ACESSO
-- =================================
