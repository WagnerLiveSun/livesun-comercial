            ...
            if nbs:
                ...
                if codigo_servico:
                    ...
                    if len(nbs_normalizado) >= 6 and nbs_normalizado[:6] != codigo_servico_padded:
                        import logging
                        logging.warning(f'NBS ({nbs_normalizado}) nao corresponde ao codigo fiscal ({codigo_servico_padded}), mas cadastro permitido. - comercial_operacional.py:367')
                nbs = nbs_normalizado
        ...
        try:
            ...
            if nbs:
                ...
                if codigo_servico:
                    ...
                    if len(nbs_normalizado) >= 6 and nbs_normalizado[:6] != codigo_servico_padded:
                        import logging
                        logging.warning(f'NBS ({nbs_normalizado}) nao corresponde ao codigo fiscal ({codigo_servico_padded}), mas cadastro permitido. - comercial_operacional.py:428')
                nbs = nbs_normalizado
...
def orcamentos_exportar_pdf(orcamento_id):
    ...
    try:
        ...
        for item in itens_lista:
            ...
            # Altura dinâmica baseada no tamanho da descrição (más fina, evita espaços vazios)
            linhas_desc = max(1, len(desc) // 55 + 1)
            h_row = max(8, linhas_desc * 5.5 + 3)
