...
from src.models import db, Entidade, Lancamento, NfseMunicipioReferencia
...
def criar():
    ...
    municipios = NfseMunicipioReferencia.query.filter_by(ativo=True).order_by(
        NfseMunicipioReferencia.uf_sigla.asc(),
        NfseMunicipioReferencia.nome_municipio.asc(),
    ).all()
    if request.method == 'POST':
        try:
            ...
            if existe:
                ...
                return render_template('entidades/form.html', action='criar', vendedores=vendedores, entidade=None, contas_fluxo=contas_fluxo, municipios=municipios)
            ...
            entidade = Entidade(
                ...
                codigo_municipio_ibge=municipio.codigo_ibge if municipio else None,
                endereco_cidade=municipio.nome_municipio if municipio else (request.form.get('endereco_cidade') or '').strip(),
                endereco_uf=municipio.uf_sigla if municipio else (request.form.get('endereco_uf') or '').strip().upper(),
                endereco_cep=format_cep(request.form.get('endereco_cep')),
...
def editar(id):
    ...
    if request.method == 'POST':
        try:
            ...
            entidade.codigo_municipio_ibge = municipio.codigo_ibge if municipio else entidade.codigo_municipio_ibge
            entidade.endereco_cidade = municipio.nome_municipio if municipio else (request.form.get('endereco_cidade') or '').strip()
            entidade.endereco_uf = municipio.uf_sigla if municipio else (request.form.get('endereco_uf') or '').strip().upper()
            entidade.endereco_cep = format_cep(request.form.get('endereco_cep'))
            ...
            return render_template('entidades/form.html', action='editar', entidade=entidade, vendedores=vendedores, contas_fluxo=contas_fluxo, municipios=municipios)
